...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Best ${name}  Pinstack Directory`,
    description: `Discover the top ${name.toLowerCase()} tools and products, ranked by community upvotes on Pinstack.`,
    alternates: { canonical: `https://pinstack.cc/category/${params.slug}` },
  };
}
    ...
    <>
      ...
      <main className="max-w-[1100px] mx-auto px-4 sm:px-6 py-10 sm:py-14">
        <h1 className="text-2xl font-extrabold text-heading mb-2">
          Best {category?.name || params.slug}
        </h1>
